...
public class addxyCoupon {
	...
	public static void main(String[] args) throws Exception {
		...
		read1.getExcel("D:\\Data\\xycoupon.xls", "西游首测");    //礼包信息
		for(int i=1;i<read1.rows;i++){
			...
			Thread.sleep(1000);
			driver.findElement(By.cssSelector("input[value=\"增加游戏用礼包\"]")).click();
			Thread.sleep(1000);
			...
			read2.getExcel("D:\\Data\\xycoupon.xls", "西游首测附件");          //附件信息
			for(int j=1;j<read2.rows;j++){
				...
				if(title.equals(PacketName)){
					for(int n=1;n<read2.columns;n++){
						...
						String Stars =read2.readColumn("星级"+n, j);
						String type=read2.readColumn("类别"+n, j);
						String Sort=read2.readColumn("类型"+n, j);
						addAnnex annex =new addAnnex();
						Thread.sleep(1000);
						annex.getXYAnnex(driver, itemName, Num, Stars, type, Gameid, Sort);;  //添加附件
					}
				}
			}
			 Thread.sleep(1000);
